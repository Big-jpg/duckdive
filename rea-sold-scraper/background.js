...
"use strict";
importScripts("slice.js");

const { makeSliceId, buildUrl, recordInDateRange } = globalThis.ReaSlice;
...
let state = {
  ...
  settings: {
    ...
    maxPagesPerSuburb: RESULT_WINDOW_PAGES,
    fetchRetries: 3,
...
const saveState = async () => {
  await chrome.storage.local.set({
    ...
    rateLimit: {
      ...
      nextCooldownMs: rl.nextCooldownMs,
      retiredCount: rl.retiredCount
    }
  });
};

const loadState = async () => {
  const stored = await chrome.storage.local.get(["queue", "settings", "processing", "paused", "rateLimit"]);
  if (stored.queue)    state.queue    = stored.queue;
  if (stored.settings) state.settings = { ...state.settings, ...stored.settings };
  state.settings.maxPagesPerSuburb = RESULT_WINDOW_PAGES;
  state.settings.concurrency = Math.min(2, Math.max(1, Number(state.settings.concurrency) || 1));
  if (stored.rateLimit) {
    rl.cooldownUntil = Number(stored.rateLimit.cooldownUntil) || 0;
    rl.nextCooldownMs = Math.max(60_000, Number(stored.rateLimit.nextCooldownMs) || 60_000);
    rl.retiredCount = Number(stored.rateLimit.retiredCount) || 0;
  }
  ...
  state.queue.forEach(job => {
    ...
    job.sliceId ||= makeSliceId(job);
    job.sliceName ||= job.suburb;
    job.recordsSeen ||= job.totalRecords || 0;
...
const processJobOnTab = async (tabId, job) => {
  ...
  job.recordsSeen  = job.recordsSeen  || 0;
  job.error        = null;
  ...
  const maxPage   = () => Math.min(
    job.calculatedPageCeiling || RESULT_WINDOW_PAGES,
    RESULT_WINDOW_PAGES
  );
  ...
  for (let page = startPage; page <= maxPage(); page++) {
    ...
    const pageUrl = buildUrl(job, page);
    ...
    if (response.empty || response.records.length === 0) {
      job.completionReason = "empty_page";
      break;
    }
    ...
    if (page === 1 && response.totalResults != null) {
      ...
      job.calculatedPageCeiling = Math.ceil(response.totalResults / RESULTS_PER_PAGE);
      const visiblePages = Math.min(job.calculatedPageCeiling, RESULT_WINDOW_PAGES);
      job.progressNote = `Detected ${response.totalResults} results (${visiblePages} visible pages${job.calculatedPageCeiling > RESULT_WINDOW_PAGES ? ", capped" : ""})`;
    }

    let newCount = 0;
    for (const record of (response.records || [])) {
      job.recordsSeen++;
      if (!recordInDateRange(record, job)) continue;
      record.sliceId = job.sliceId;
      record.sliceName = job.sliceName;
      record.planRunId = job.planRunId || null;
  ...
  job.status       = "complete";
  if (!job.completionReason) {
    if (job.currentPage >= RESULT_WINDOW_PAGES) job.completionReason = "result_window_reached";
    else if (job.calculatedPageCeiling && job.currentPage >= job.calculatedPageCeiling) job.completionReason = "reported_total_reached";
    else job.completionReason = "duplicate_pages";
  }
  job.totalRecords = allRecords.length;
  await chrome.storage.local.set({ [storageKey]: allRecords });
  tabPool.set(tabId, null);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "ADD_JOB") {
    const slice = {
      ...
      includeSurrounding: message.includeSurrounding === true,
      bedroomsMin: message.bedroomsMin ?? null,
      bedroomsMax: message.bedroomsMax ?? null,
      priceMin: message.priceMin ?? null,
      priceMax: message.priceMax ?? null,
      maxSoldAge: message.maxSoldAge || null,
      soldDateFrom: message.soldDateFrom || null,
      soldDateTo: message.soldDateTo || null
    };
    const sliceId = makeSliceId(slice);
    const duplicate = state.queue.find(job => job.sliceId === sliceId && job.status !== "error");
    if (duplicate) {
      sendResponse({ success: true, duplicate: true, job: duplicate });
      return false;
    }
    const job = {
      id:                    `${sliceId}_${Date.now()}`,
      sliceId,
      sliceName:             message.sliceName || message.suburb,
      planRunId:             message.planRunId || null,
      ...slice,
      status:                "pending",
      currentPage:           0,
      totalRecords:          0,
  ...
  if (message.type === "UPDATE_SETTINGS") {
    state.settings = {
      ...
      ...message.settings,
      maxPagesPerSuburb: RESULT_WINDOW_PAGES,
      concurrency: Math.min(2, Math.max(1, Number(message.settings?.concurrency) || 1))
    };
    saveState().then(() => broadcastState());
    sendResponse({ success: true });
    return false;
