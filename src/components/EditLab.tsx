...
type EditorDive={key:string;datasetKey:string;datasetTitle:string;title:string;label:string;description:string;outcome:string;entryPrompt:string;questions:string[];accent:string;diveId:string;contractVersion:string;publicContract:DatasetPublicContract;publicShare:boolean};
...
function DuckDivePanel({diveId,starterKey,contract,report,active,version,onApplied,onError}:{diveId:string;starterKey:string;contract:DatasetPublicContract;report:ReportVersionMetadata|null;active:boolean;version:number|null;onApplied:(version:number)=>Promise<void>;onError:(message:string)=>void}){
  ...
  return <aside className="chat-panel" hidden={!active}>
    ...
    <nav className="report-tabs" aria-label="Report explanation">{([["summary","Summary","Summary"],["changes","Changes","Changes"],["ideas","Ideas","Ideas for This Report"],["contract","Contract","Technical Contract"]] as const).map(([key,label,ariaLabel])=><button key={key} aria-label={ariaLabel} aria-pressed={tab===key} onClick={()=>setTab(key)}>{label}</button>)}</nav>
...
export default function EditLab(){
  ...
  async function refresh(id=activeRef.current){
    if(!id)return;setError("");setRefreshing(true);
    try{
      ...
      if(!embedResponse.ok){setError(embedBody.error||"The report preview is unavailable. Try loading it again.");return;}
    ...
    }catch{setError("The report could not be reached. Check your connection and try again.");}
  ...
  return <main id="main-content" className="edit-lab">
    <header className="lab-header edit-header"><AppBrand/><nav className="edit-tabs" aria-label="Choose a report">{dives.map(dive=><button aria-pressed={activeDive?.key===dive.key} key={dive.key} className={activeDive?.key===dive.key?"active":""} onClick={()=>updateEditorState(dive.key,pane)}>{dive.label}</button>)}</nav><Link className="edit-close" href="/workspace">Back to Workspace</Link></header>
    <nav className="edit-pane-tabs" aria-label="Choose an editor pane"><button aria-pressed={pane==="report"} onClick={()=>activeDive&&updateEditorState(activeDive.key,"report")}>Report Preview</button><button aria-pressed={pane==="duckdive"} onClick={()=>activeDive&&updateEditorState(activeDive.key,"duckdive")}>Change with DuckDive</button></nav>
    <div className="edit-grid">
      <section className="edit-canvas" data-mobile-visible={pane==="report"} aria-label="Report preview"><div className="edit-toolbar"><div className="edit-context"><span>{activeDive?.datasetTitle||"Workspace"}</span><strong>{activeDive?.title||"Preparing Your Reports…"}</strong><small><i aria-hidden="true"/>{version?`Version ${version}`:"Loading current version"}{share?` · Shared · ${share.viewCount.toLocaleString("en-AU")} views`:""}</small></div><div className="edit-actions">{activeDive?.publicShare?(share?<><button disabled={sharing} onClick={copyShare}>Copy Share Link</button><button disabled={sharing} onClick={revokeShare}>Revoke Link</button></>:<button disabled={!activeId||sharing} onClick={publish}>{sharing?"Publishing…":"Publish Share Link"}</button>):null}<button disabled={!activeId||refreshing} onClick={()=>refresh()}>{refreshing?"Refreshing…":"Refresh Report"}</button><button disabled={!version||version<=1||reverting} onClick={revert}>{reverting?"Restoring…":"Restore Previous Version"}</button></div>{shareStatus?<p className="edit-status" role="status" aria-live="polite">{shareStatus}</p>:null}</div>{error?<div className="lab-error edit-state" role="alert" aria-live="assertive"><strong>{activeId?"Report Unavailable":"Workspace Unavailable"}</strong><span>{error}</span><button disabled={refreshing} onClick={()=>activeId?refresh():location.reload()}>{refreshing?"Trying Again…":activeId?"Try Again":"Reload Workspace"}</button></div>:embed?<iframe key={embed} title={activeDive?`${activeDive.title} report`:"Editable report"} src={`https://embed-motherduck.com/sandbox/#session=${embed}`} sandbox="allow-scripts allow-same-origin"/>:<div className="lab-loading edit-state" role="status" aria-live="polite"><strong>Preparing Report…</strong><span>Loading the governed report and its current version.</span></div>}</section>
