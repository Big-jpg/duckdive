...
export default function LoginForm(){
  ...
  return <main id="main-content" className="login-page"><AppBrand/><form onSubmit={submit}><p>DuckDive Access</p><h1>Continue to the lab</h1><span>DuckDive is currently open to invited testers. Use GitHub or request a secure, single-use link by email.</span><button type="button" className="github-login" disabled={githubLoading||loading} onClick={github}>{githubLoading?"Opening GitHub…":"Continue with GitHub"}</button><div className="login-divider"><span>or use email</span></div><label>Email<input name="email" type="email" value={email} onChange={event=>setEmail(event.target.value)} required autoComplete="email" maxLength={254} spellCheck={false}/></label>{error&&<div className="lab-error" role="alert">{error}</div>}{message&&<div className="lab-success" role="status" aria-live="polite">{message}</div>}<button disabled={loading||githubLoading||Boolean(message)}>{loading?"Sending…":message?"Link Requested":"Send a Sign-In Link"}</button><span className="login-help">Have an unlisted Dive link? Open it directly—no account required.</span></form></main>;
}
