...
export default function LoginForm(){
  ...
  return <main id="main-content" className="login-page"><AppBrand/><form onSubmit={submit}><p className="lab-kicker">Governed analytics workspace</p><h1>Sign In</h1><p className="login-intro">Access your owner-scoped datasets, Dives, editing, versions, and shares.</p><button type="button" className="github-login" disabled={githubLoading||loading} onClick={github}>{githubLoading?"Opening GitHub…":"GitHub"}</button><div className="login-divider"><span>or</span></div><label>Email<input name="email" type="email" value={email} onChange={event=>setEmail(event.target.value)} required autoComplete="email" maxLength={254} spellCheck={false}/></label>{error?<div className="lab-error" role="alert">{error}</div>:null}{message?<div className="lab-success" role="status" aria-live="polite">{message}</div>:null}<button disabled={loading||githubLoading||Boolean(message)}>{loading?"Sending…":message?"Link Sent":"Email a Sign-In Link"}</button><p className="login-request">Not approved yet? <Link href="/">Request access.</Link></p></form></main>;
}
