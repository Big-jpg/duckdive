...
export default function OperationalCandidatePreview({candidate,onClose}:{candidate:OperationalDatasetCandidateV1;onClose:()=>void}){
  return <div className="byod-activation-preview">
    <header><div><p className="lab-kicker">Activation preview · not registered</p><strong>{candidate.identity.displayName}</strong><small>{candidate.identity.candidateKey}</small></div><button type="button" onClick={onClose}>Close</button></header>
    <div className="byod-preview-status"><span>Runtime unbound</span><span>No SQL generated</span><span>Nothing persisted</span></div>
