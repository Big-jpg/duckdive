...
export function aiModel(provider?:string){
  ...
  if(selected==="gateway"){if(!process.env.VERCEL&&!process.env.AI_GATEWAY_API_KEY&&!process.env.VERCEL_OIDC_TOKEN)throw new Error("AI Gateway authentication is not configured");return duckDiveModelName();}
  if(!process.env.OPENAI_API_KEY)throw new Error("OPENAI_API_KEY is not configured");
  return openai(process.env.AI_MODEL_OPENAI||"gpt-5.4");
}

export function duckDiveModelName(){return process.env.AI_MODEL_GATEWAY||"openai/gpt-5.6-sol";}
