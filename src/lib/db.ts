...
export async function registerFile(fileName: string, objectUrl: string | null, sha256: string, byteSize: number) {
  ...
  try {
    const metadata=sourceFileMetadata(fileName);
    const [file] = await sql`INSERT INTO ops.ingest_file(file_name,object_url,sha256,byte_size,source_state,source_postcode)
      VALUES(${fileName},${objectUrl},${sha256},${byteSize},${metadata.state},${metadata.postcode}) ON CONFLICT(sha256) DO UPDATE SET object_url=coalesce(excluded.object_url,ops.ingest_file.object_url)
...
export async function curateFile(fileId: string) {
  const sql=directDatabase(); try { await sql`CALL core.curate_file(${fileId}::uuid)`; } finally { await sql.end(); }
}

export async function refreshMarts() {
  const sql=directDatabase(); try {
