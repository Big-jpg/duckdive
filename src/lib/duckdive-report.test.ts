...
describe("DuckDive report explanation contract",()=>{
  it("derives capabilities and limitations from the active dataset policy",()=>{
    ...
    expect(ids).toContain("report-presentation");
  ...
  it("rejects failed plan validation for an allowlisted presentation change",()=>{
    const plan={request:"Use brighter colours",interpretedIntent:"Increase chart contrast",purpose:purpose(),capabilityIds:["report-presentation"],unsupportedRequests:[],materialClarification:null,added:[],changed:["Chart colours"],removed:[],unchanged:[],validations:[{id:"palette",label:"Palette validation",status:"failed" as const,detail:"Governed semantics were not preserved"}]};
    expect(validateReportUpdatePlan(plan,policy).ok).toBe(false);
    expect(validateReportUpdatePlan({},policy).ok).toBe(false);
  });
