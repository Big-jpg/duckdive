...
export function propertyFingerprint(address: string, postcode = "", state = process.env.ESTATE_STATE||"VIC") {
  return sha256(`${normaliseAddress(address)}|${state.toUpperCase()}|${postcode}`);
}
