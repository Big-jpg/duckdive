...
export default async function SharedDivePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main id="main-content" className="shared-dive-page"><header className="lab-header"><AppBrand/><span className="shared-badge">Read Only</span></header><section className="dive-title"><h1>{share.title}</h1></section><iframe className="dive-full" title={share.title} src={`https://embed-motherduck.com/sandbox/#session=${session}`} sandbox="allow-scripts allow-same-origin"/></main>;
}
