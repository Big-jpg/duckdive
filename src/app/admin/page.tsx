...
const labels:Record<string,string>={"ai.completed":"AI completed","duckdive.applied":"DuckDive saved","duckdive.clarification":"DuckDive asked for detail","duckdive.failed":"DuckDive failed","duckdive.aborted":"DuckDive stopped","duckdive.reset":"Starter restored","duckdive.reset.no_change":"Starter already current","dive.share.published":"Share published","dive.share.revoked":"Share revoked","workspace.provisioned":"Workspace created","admin.access.upserted":"Access updated","admin.access.revoked":"Access revoked","admin.access_request.approved":"Access request approved","admin.access_request.ignored":"Access request ignored","auth.logout":"Signed out"};

export default async function AdminPage(){
  const user=await currentUser();if(!user)redirect("/login?next=%2Fadmin");if(user.role!=="admin")notFound();
  const [{overview,users,audit},requests]=await Promise.all([getAdminDashboard(),listPendingAccessRequests()]),limits=aiLimits(),shareLimits=publicShareLimits();
  const cards=[
