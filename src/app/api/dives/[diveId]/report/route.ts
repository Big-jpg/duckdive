...
export async function GET(request:Request,{params}:{params:Promise<{diveId:string}>}){
  ...
  try{
    ...
    const fallback=matchesStarter?starterReportVersion({workspaceId:owned.workspace_id,diveId,version:snapshot.version,sourceHash:snapshot.hash,starterKey:starter.key,title:starter.title,description:starter.description,contract:context.dataset.publicContract}):null;
    try{
