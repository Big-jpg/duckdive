...
export async function GET(request:Request,{params}:{params:Promise<{diveId:string}>}){
  ...
  const share=await getActiveDiveShare(result.workspaceDive.workspace_id,diveId);
...
export async function POST(request:Request,{params}:{params:Promise<{diveId:string}>}){
  ...
  const {diveId}=await params,result=await owned(request,diveId);if("error" in result)return result.error;
  const share=await createDiveShare(result.workspaceDive.workspace_id,result.user.user_id,diveId,result.starter.key,result.starter.title,result.starter.description);
...
export async function DELETE(request:Request,{params}:{params:Promise<{diveId:string}>}){
  const csrf=assertSameOrigin(request);if(csrf)return csrf;
  const {diveId}=await params,result=await owned(request,diveId);if("error" in result)return result.error;
  const share=await revokeDiveShare(result.workspaceDive.workspace_id,diveId);if(!share)return Response.json({error:"No active share link"},{status:404});
