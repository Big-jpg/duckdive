...
export async function POST(request:Request,{params}:{params:Promise<{diveId:string}>}){
  ...
  const starter=dataset.dataset.starters.find(item=>item.key===starterKey);if(!starter)return Response.json({error:"Starter report unavailable"},{status:404});
  ...
  try{
    ...
    let report=null;
    if(!result.noChange){
      const purpose=reportPurposeForStarter({title:starter.title,description:starter.description,policy:dataset.dataset.reportPolicy});
      try{report=await saveDiveReportVersion({workspaceId:workspaceDive.workspace_id,diveId,version:result.after.version,sourceHash:result.after.hash,purpose,manifest:{request:"Reset report",interpretedIntent:"Restore the registered starter report",requested:{added:["Starter report restored"],changed:[],removed:[],unchanged:[]},applied:{added:["Verified starter report source"],changed:[],removed:[],unchanged:["Governed data contract"]},validations:[{id:"reset-starter",label:"Starter report was restored",status:"passed"}],version:result.after.version,generatedAt:new Date().toISOString()}});}catch(error){console.error("Reset report metadata persistence failed",error);}
    }
    try{await audit(result.noChange?"duckdive.reset.no_change":"duckdive.reset",user.user_id,diveId,{beforeVersion:result.before.version,afterVersion:result.after.version,starterKey});}catch(error){console.error("Dive reset audit failed",error);}
