...
export async function POST(request:Request){
  ...
  const result=await neonAuth().signIn.social({provider:"github",callbackURL:appUrl(`/auth/complete?next=${encodeURIComponent(next)}`,request.url),errorCallbackURL:appUrl("/login?error=github_failed",request.url),disableRedirect:true});
