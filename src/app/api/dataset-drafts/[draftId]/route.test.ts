...
vi.mock("@/lib/dataset-drafts-db",()=>({DatasetDraftInUseError:class DatasetDraftInUseError extends Error{},getDatasetDraft:mocks.getDatasetDraft,deleteDatasetDraft:mocks.deleteDatasetDraft}));
vi.mock("@/lib/app-db",()=>({audit:mocks.audit}));

import {DELETE,GET} from "./route";
import {DatasetDraftInUseError} from "@/lib/dataset-drafts-db";
...
describe("dataset draft item route",()=>{
  ...
  it("preserves evidence linked to an activated dataset",async()=>{mocks.deleteDatasetDraft.mockRejectedValue(new DatasetDraftInUseError("Activated dataset evidence cannot be deleted"));const response=await DELETE(request("DELETE"),context);expect(response.status).toBe(409);expect(mocks.audit).not.toHaveBeenCalled();});
});
