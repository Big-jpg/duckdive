...
For current multi-dataset work, read `MULTI_DATASET_DELIVERY_PLAN.md` for intended scope and sequence. Read the newest checkpoint at the top of `NEXT_SESSION_HANDOFF.md` for verified implementation and production state. The handoff wins on completed evidence; the delivery plan defines the next gate. Do not redo completed work unless read-only verification shows drift.
