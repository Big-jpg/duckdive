-- Keep Neon marts reusable. Estate-specific state/property scope is applied by the publisher.
...
SELECT d.suburb_key,d.state,d.suburb,d.canonical_postcode postcode,initcap(s.property_type) property_type,
 date_trunc('month',s.sold_date)::date sale_month,count(*)::bigint sale_count,
...
GROUP BY d.suburb_key,d.state,d.suburb,d.canonical_postcode,initcap(s.property_type),date_trunc('month',s.sold_date)::date WITH NO DATA;
CREATE UNIQUE INDEX suburb_monthly_sales_key ON mart.suburb_monthly_sales(suburb_key,property_type,sale_month);
