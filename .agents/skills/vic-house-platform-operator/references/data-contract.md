...
Public interfaces are `/api/stats`, `/api/analytics/suburbs`, `/api/analytics/suburb-sales`, `/api/analytics/suburb-insights`, `/api/gallery`, unlisted `/share/[slug]`, and secret-protected `POST /api/ingest`. Authenticated owners manage links through `/api/dives/[diveId]/share`. Never accept arbitrary public SQL or public Dive IDs; keep routes, parameters, metrics, filters, and result sizes allowlisted and bounded.
